...
public class SaveAndGetCustomerTest {
    ...
    @Test
    public void test1SaveAndGetCustomer(TestContext context) throws Exception {
        Async async = context.async();
        JsonNode source = JsonLoader.fromResource("/Customer.json");
        JsonObject clone = new JsonObject(Json.encode(source));
        clone.getJsonObject("personalDetails").put("phoneNumber", ((Long.parseLong(clone.getJsonObject("personalDetails").getString("phoneNumber")) + 1) + ""));
        customerRepository.create(Json.encode(clone), r -> {
            AsyncResult<String> result = (AsyncResult<String>) r;
            if (result.failed()) {
                result.cause().printStackTrace();
                logger.info("WRONG: " + result.cause().getMessage());
                context.assertTrue(result.succeeded());
                async.complete();
                return;
            }
            customerRepository.get(result.result(), rr -> {
                if (rr.succeeded()) {
                    try {
                        ...
                        if (!Json.encode(rr.result()).equals(encoded)) {
                            logger.info(loaded.getId() + " - SOURCE : " + encoded);
                            logger.info(loaded.getId() + " - RESULT : " + Json.encode(rr.result()));
                        }
